...
As I was working around to create my own e-mail server (mostly to set a e-mail bot to sent me Dilbert comics everyday), I found a pretty annoying bug on my router's webapp:
...
Because I do not expect everyone to feel comfortable around a terminal! Or maybe not even wanting to use it!
The internet should be a **democratic space, accessible to all**! I think a webapp/GUI is much more friendly to the common user/newb.

Althought this can be seen as an exercise in learning - This does not excuse an ISP to remove features, much less features that without them its users can be harmed.

Additionally this shows one of the nastiest things - We the people, are not in control of the technology we are using, first they remove the WebApp, one day the (already stunted) terminal interface, and finally port forwarding in total.
