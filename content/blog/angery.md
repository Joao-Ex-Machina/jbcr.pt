...
As I was working around to create my own e-mail server (mostly to set up an e-mail bot to send me Dilbert comics every day), I found a pretty annoying bug on my router's webapp:

![Error on IPv4 Port Forwarding](/pf.png)

The access to IPv4 port forwarding was blocked. Supposedly a bug on the firmware - a bug that has been going on for over 4 months now, with no solution in sight.
...
show # show current forwarding or device-info
...
Although this can be seen as an exercise in learning - This does not excuse an ISP to remove features, much less features that without them their users can be harmed.
